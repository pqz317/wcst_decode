...
def get_trial_intervals(behavioral_data, event="FeedbackOnset", pre_interval=0, post_interval=0):
    ...
    :param behavioral_data: Dataframe describing each trial, 
        must contain columns: TrialNumber, whatever 'event' param describes
    :param event: name of event to align around, must be present as a column name in behavioral_data Dataframe
    :param pre_interval: number of miliseconds before event
    :param post_interval: number of miliseconds after event
...
def get_selection_features(behavioral_data):
    ...
    :param behavioral_data: Dataframe describing each trial, must contain columns: 
        - ItemChosen, either 0, 1, 2, 3, index of item that was chosen that trial
        - For each idx 0, 1, 2, 3, columns 'Item<idx><feature>', where feature is 'Color', 'Pattern' or 'Shape'
    :returns: DataFrame with num_trials length, columns: TrialNumber, Color, Pattern, Shape
    """
    selections = []
