...
def generate_figure(base_dir):
    ...
    for feature_dim in FEATURE_DIMS:
        ...
        if not os.path.isdir(fig_dir):
            os.mkdir(fig_dir)
        plt.savefig(os.path.join(fig_dir, f"{feature_dim}_decoding.png"))
...
def decode_features(row):
    ...
    if not REPLACE and check_decoder_already_ran():
        print(f"Feature Decoders for {sess_name} has already run, skipping")
    else: 
        print(f"Running Feature Decoders for {sess_name}")
        run_decoder(frs, feature_selections, base_dir)
    # generate figures
    if not REPLACE and check_figures_already_generated():
        print(f"Feature Figures for {sess_name} has already been generated, skipping")
    else: 
