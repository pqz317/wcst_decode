...
def run_decoder(frs, valid_beh, base_dir):
    """
    ...
    outputs = classifier_utils.evaluate_classifiers_by_time_bins(
        wrapped, inputs, labels, bins, random_splitter
    )
    io_utils.save_model_outputs(
        "feedback", 
        f"{PRE_INTERVAL}_{EVENT}_{POST_INTERVAL}",
        "random",
        outputs,
        base_dir=base_dir
    )
...
def generate_figure(base_dir):
    ...
    if not os.path.isdir(fig_dir):
        os.mkdir(fig_dir)
    plt.savefig(os.path.join(fig_dir, "feedback_decoding.png"))
...
def decode_feedback(row):
    ...
    if not REPLACE and check_decoder_already_ran():
        print(f"Feedback Decoder for {sess_name} has already run, skipping")
    else: 
        print(f"Running Decoder for {sess_name}")
        run_decoder(frs, valid_beh, base_dir)
    # generate figures
    if not REPLACE and check_figures_already_generated():
        print(f"Feedback Figures for {sess_name} has already been generated, skipping")
    else: 
...
def main():
    ...
    valid_sess.apply(decode_feedback, axis=1)
