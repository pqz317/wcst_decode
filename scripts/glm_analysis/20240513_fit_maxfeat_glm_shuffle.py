...
def calc_and_save_session(sess_name, splits, model, norm_mode, shuffle_idx):
    start = time.time()
    ...
    # NOTE: hacky, previously it was MaxFeat, but shouldn't matter since current rule == maxfeat
...
def main():
    ...
    valid_sess.apply(lambda row: calc_and_save_session(row.session_name, splits, args.model, args.norm_mode, args.shuffle_idx), axis=1)
