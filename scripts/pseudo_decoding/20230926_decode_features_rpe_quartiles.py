...
def load_session_data(row, condition): 
    ...
    pos = row.FE_quantile_25
    quantile_75 = row.FE_quantile_75
    low_rpe_beh = valid_beh_rpes[valid_beh_rpes.RPE_FE < quantile_25]
    high_rpe_beh = valid_beh_rpes[valid_beh_rpes.RPE_FE > quantile_75]
    print(f"{len(low_rpe_beh)} trials for low RPE, {len(high_rpe_beh)} trials for high RPE")
    all_rpe_beh = pd.concat((low_rpe_beh, high_rpe_beh))
    ...
    all_splitter = ConditionTrialSplitter(all_rpe_beh, condition, 0.2)
    return (
        SessionData(sess_name, low_rpe_beh, frs, low_splitter),
        SessionData(sess_name, high_rpe_beh, frs, high_splitter),
        SessionData(sess_name, all_rpe_beh, frs, all_splitter),
    )
...
def decode_feature(feature_dim, valid_sess):
    ...
    run_decoder(low_datas, feature_dim, "low_quartile_rpes")
    run_decoder(high_datas, feature_dim, "high_quartile_rpes")
    run_decoder(all_datas, feature_dim, "low_high_quartile_rpes")
