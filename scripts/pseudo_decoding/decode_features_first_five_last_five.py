...
def load_session_data(sess_name, condition, mode): 
    """
    ...
    splitter = ConditionTrialSplitter(valid_beh_merged, condition_column=condition, test_ratio=0.2)
...
def decode_feature(feature_dim, valid_sess, mode):
    ...
    np.save(os.path.join(base_dir, f"{mode}_{feature_dim}_train_accs.npy"), train_accs)
    np.save(os.path.join(base_dir, f"{mode}_{feature_dim}_test_accs.npy"), test_accs)
    np.save(os.path.join(base_dir, f"{mode}_{feature_dim}_shuffled_accs.npy"), shuffled_accs)
    np.save(os.path.join(base_dir, f"{mode}_{feature_dim}_models.npy"), models)


def main():
    valid_sess = pd.read_pickle("/data/patrick_scratch/multi_sess/valid_sessions.pickle")
    for feature_dim in feature_dims: 
        decode_feature(feature_dim, valid_sess, "first_five")
        decode_feature(feature_dim, valid_sess, "last_five")
