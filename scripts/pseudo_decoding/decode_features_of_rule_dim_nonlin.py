# same thing as decode features of rule dim, but using nonlinear model
...
from models.multinomial_logistic_regressor import NormedDropoutNonlinear
...
def decode_features_of_rule_dim(rule_dim, valid_sess):
    ...
    for feature_dim in feature_dims:
        ...
        init_params = {"n_inputs": num_neurons, "p_dropout": 0.5, "n_classes": len(classes), "hidden_sizes":[100, 100]}
        ...
        np.save(os.path.join(base_dir, f"rule_dim_{rule_dim}_{feature_dim}_nonlin_decoding_train_accs.npy"), train_accs)
        np.save(os.path.join(base_dir, f"rule_dim_{rule_dim}_{feature_dim}_nonlin_decoding_test_accs.npy"), test_accs)
        np.save(os.path.join(base_dir, f"rule_dim_{rule_dim}_{feature_dim}_nonlin_decoding_shuffled_accs.npy"), shuffled_accs)
        np.save(os.path.join(base_dir, f"rule_dim_{rule_dim}_{feature_dim}_nonlin_decoding_models.npy"), models)
