...
def load_session_data(row, feat_a, feat_b, should_shuffle=False, shuffle_seed=None, norm_type=None):
    ...
    a_not_enough = beh[beh.CurrentRule == feat_a].BlockNumber.nunique() < NUM_BLOCKS_PER_COND
    b_not_enough = beh[beh.CurrentRule == feat_b].BlockNumber.nunique() < NUM_BLOCKS_PER_COND
    if a_not_enough or b_not_enough:
        return None
    
    if should_shuffle:
        beh = behavioral_utils.shuffle_block_rules(beh, seed=shuffle_seed)
    beh = beh[beh.CurrentRule.isin([feat_a, feat_b])]
    ...
    splitter = ConditionTrialSplitter(beh, "CurrentRule", TEST_RATIO, seed=DECODER_SEED)
    session_data = SessionData(sess_name, beh, frs, splitter)
    session_data.pre_generate_splits(NUM_SPLITS)
    return session_data

def decode(valid_sess, feat_a, feat_b, should_shuffle, shuffle_seed, norm_type, save_sess_datas):
    sess_datas = valid_sess.apply(lambda x: load_session_data(x, feat_a, feat_b, should_shuffle, shuffle_seed, norm_type), axis=1)
    sess_datas = sess_datas.dropna()
    print(f"decoding {feat_a} from {feat_b} with {len(sess_datas)} sessions, norm type {norm_type}")
    ...
    model = ModelWrapper(NormedDropoutMultinomialLogisticRegressor, init_params, trainer, [feat_a, feat_b])
    ...
    # store the results
    run_name = f"max_feat_{feat_a}_{feat_b}_{shuffle_str}{norm_str}"
