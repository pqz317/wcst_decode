...
for trial_event in $trial_events; do
    ...
    submit_job_array "0-27" "${trial_event}" \
        "--trial_event $trial_event --mode pref --sig_unit_level pref_99th_window_filter_drift --pair_idx \$SLURM_ARRAY_TASK_ID"

    # Second job array: 280 jobs with shuffle indices
    submit_job_array "0-279" "sh${trial_event}" \
        "--trial_event $trial_event --mode pref --sig_unit_level pref_99th_window_filter_drift --pair_idx \$((\$SLURM_ARRAY_TASK_ID % 28)) --shuffle_idx \$((\$SLURM_ARRAY_TASK_ID / 28))"
done