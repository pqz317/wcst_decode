...
singularity exec --writable-tmpfs --nv \
    ...
    /src/wcst_decode/scripts/pseudo_decoding/belief_partitions/ccgp_feat_beliefs.py $python_args $extra_args
EOT
}
...
for trial_event in $trial_events; do
    ...
    submit_job_array "0-27" "${trial_event}" \
        "--trial_event $trial_event --mode pref --sig_unit_level pref_99th_window_filter_drift --pair_idx \$SLURM_ARRAY_TASK_ID --base_output_path /data/patrick_res/ccgp_pref_new"

    # Second job array: 280 jobs with shuffle indices
    submit_job_array "0-279" "sh${trial_event}" \
        "--trial_event $trial_event --mode pref --sig_unit_level pref_99th_window_filter_drift --pair_idx \$((\$SLURM_ARRAY_TASK_ID % 28)) --shuffle_idx \$((\$SLURM_ARRAY_TASK_ID / 28)) --base_output_path /data/patrick_res/ccgp_pref_new"
done