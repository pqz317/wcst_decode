...
# the all-units choice config behind /data/patrick_res/choice_reward
CONTRASTS = {
...
def vectors_for_session(sess_name, args, feats_for_sess):
    """
    ...
    Firing rates and behavior are each read once, since with no subpopulation selection the unit
    set doesn't depend on the feature.
    ...
    if len(raw_beh) == 0 or len(frs) == 0:
        print(f"session {sess_name}: no behavior or firing rates, skipping", flush=True)
        return None, []
...
def label_regions(vectors):
    """
    Adds a `region` column to the per-unit vectors, resolved the same way the decoding runs select
    regions (spike_utils.get_all_region_units on REGION_LEVEL).
    ...
    vectors["region"] = vectors.PseudoUnitID.map(region_of)
...
def align_for_sub(args):
    """
    Runs one subject's sessions, returning per-unit vectors and per-condition trial counts.
    """
    ...
    for sess_name in sub_sessions.session_name:
        ...
        vectors, trial_counts = vectors_for_session(sess_name, args, feats_for_sess)
...
def align(args):
    ...
    print(f"\nsaved {len(summary)} (region, feat, TimeIdx) rows to {output_dir}", flush=True)
...
def process_args(args):
    ...
    print(f"Aligning {list(CONTRASTS)} population vectors for {args.trial_event}", flush=True)
    print(f"Contrasts: {CONTRASTS}", flush=True)
    if args.shuffle_idx is not None:
        print(f"Shuffle {args.shuffle_idx}, method {args.shuffle_method}", flush=True)
    return args
