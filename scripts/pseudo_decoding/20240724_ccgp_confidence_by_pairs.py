...
def decode(sessions, row, sim_noise=None):
    ...
    for feat in pair:
        ...
        np.save(os.path.join(OUTPUT_DIR, f"ccgp_confidence_fixed_{event_str}_pair_{pair_str}_feat_{feat}_models.npy"), models)

    within_cond_accs = np.hstack(within_cond_accs)
    across_cond_accs = np.hstack(across_cond_accs)
    np.save(os.path.join(OUTPUT_DIR, f"ccgp_confidence_fixed_{event_str}_pair_{pair_str}_within_dim_accs.npy"), within_cond_accs)
    np.save(os.path.join(OUTPUT_DIR, f"ccgp_confidence_fixed_{event_str}_pair_{pair_str}_across_dim_accs.npy"), across_cond_accs)
